define([
  ...
  function sendSampleRequest(group, name, version, type) {
    ...
    function displaySuccess(data, status, jqXHR) {
      ...
      if (name === 'Authenticate' && group === "Auth") {
        if (Object.keys(JSON.parse(jsonResponse)).includes('data')) {
          if (Object.keys(JSON.parse(jsonResponse).data).includes('token')) {
            document.querySelectorAll('input[data-sample-request-header-name="x-access-token"]').forEach((input, i) => {
              input.value = JSON.parse(jsonResponse).data.token
            })
          }
