...
require([
  ...
  function jsonifyInputObject() {

    document.querySelectorAll('.sample-request-param-type-Object').forEach((input, i) => {
      ...
      input.addEventListener("keyup", event => {
        ...
        try {
          event.target.value = JSON.stringify(JSON.parse(event.target.value));
        } catch (err) {
          return;
        }
      ...
      let label = input.closest('.label-and-input').querySelector('label');

      function inputBlur() {
        if(input.value === ""){
          label.querySelector('.text-success').classList.add('hidden')
          label.querySelector('.text-danger').classList.add('hidden');
          return;
        }
        try {
          JSON.parse(input.value);
          label.querySelector('.text-success').classList.remove('hidden')
          label.querySelector('.text-danger').classList.add('hidden')
          return;
        } catch (err) {
          label.querySelector('.text-success').classList.add('hidden')
          label.querySelector('.text-danger').classList.remove('hidden')
          return;
        }
      }
